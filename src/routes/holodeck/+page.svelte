<script>
	...
	function handleNext(first = false) {
		const next = queue.shift();
		if (!next) return;
		const duration = first ? 0 : Math.min(Math.max(next.body.length * 40, 2500), 5_000);
	...
	const viewpoint = 'Captain Picard';
...
<svelte:head>
	<title>Captain Picard in the holodeck with ChatGPT</title>
	<meta
		name="description"
		content="A fictional conversation between Captain Picard in the holodeck with ChatGPT"
	/>
</svelte:head>

<main bind:this={main}>
	<h1>To: <span>ChatGPT<span /></span></h1>
